...
int main(int argc, char *argv[]) {
  ...
  printf("WG size of kernel = %d \n", NUMBER_THREADS);
	...
	// pointers
        common_change.d_frame =
            (float *)sycl::malloc_device(common.frame_mem, q_ct1);
	...
	for(i=0; i<common.allPoints; i++){
                unique[i].d_in2_sqr =
                    (float *)sycl::malloc_device(common.in2_sqr_mem, q_ct1);
        }
	...
	for(i=0; i<common.allPoints; i++){
                unique[i].d_in_sqr =
                    (float *)sycl::malloc_device(common.in_sqr_mem, q_ct1);
        }
	...
	for(common_change.frame_no=0; common_change.frame_no<frames_processed; common_change.frame_no++){
		...
		// copy frame to GPU memory
                ...
                // launch GPU kernel
                /*
                DPCT1049:34: The workgroup size passed to the SYCL kernel may
